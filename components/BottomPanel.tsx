...
interface Props {
  ...
  onClose: () => void;
  /** Cuando true: renderiza como elemento de flujo (panel superior mobile). Cuando false: position:fixed solo en desktop. */
  mobileInline?: boolean;
}

export default function BottomPanel({
  materia, plan, estadoActual, nota, prerequisitosFaltantes, onSetEstado, onClose, mobileInline,
}: Props) {
  ...
  // ── Mobile inline: panel de flujo encima del mapa ──
  if (mobileInline) {
    return (
      <div
      ...
      >
        {/* Título + meta */}
        <div style={{ flexShrink: 0, padding: '12px 48px 0 16px' }}>
          <p style={{
        ...
        {/* Acciones */}
        <div style={{ flexShrink: 0, padding: '10px 16px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {estadoActual === null && (
            <>
              {hayFaltantes && (
                <div style={{
        ...
        {/* Correlativas — scrollable si son muchas */}
        <div
          data-scrollable-panel="true"
          style={{
            flex: 1, minHeight: 0,
  ...
  // ── Desktop: panel fijo abajo, solo se muestra en sm+ ──
  return (
    <div
      data-scrollable-panel="true"
      onMouseDown={(e) => e.stopPropagation()}
      ...
      style={{
        ...
        gap: 24, padding: '14px 44px 18px 20px',
      }}
    >
