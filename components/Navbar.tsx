      ...
      >
        <div className="flex h-full items-center gap-3 px-4">
          ...
          <div className="relative hidden sm:block">
            <div style={{
              ...
              {query && (
                <button
                  ...
                  style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', color: 'rgba(255,255,255,0.25)', fontSize: 16, lineHeight: 1, flexShrink: 0 }}
                >
                  ×
                </button>
              )}
            ...
            {showDropdown && (
              <SearchDropdown results={results} onSelect={selectResult} />
            )}
          </div>
          ...
          <button
            ...
            style={{
              background: 'rgba(45,82,105,0.15)', border: '1px solid rgba(45,82,105,0.3)',
              borderRadius: 4, width: 34, height: 34, cursor: 'pointer', flexShrink: 0,
            }}
          >
            <svg width="14" height="14" viewBox="0 0 12 12" fill="none">
              <circle cx="5" cy="5" r="3.5" stroke="rgba(255,255,255,0.5)" strokeWidth="1.3" />
              <path d="M8 8l2.2 2.2" stroke="rgba(255,255,255,0.5)" strokeWidth="1.3" strokeLinecap="round" />
            </svg>
          </button>
          ...
          <div className="relative">
            ...
            >
              <span className="hidden sm:inline">{carreras.find(c => c.slug === currentSlug)?.nombre ?? plan.nombre}</span>
              <span className="sm:hidden">{carreras.find(c => c.slug === currentSlug)?.nombre.replace(/^Ing\. (en )?|^Lic\. /, '') ?? plan.nombre}</span>
              <svg
      ...
      {mobileSearchOpen && (
        ...
        >
          ...
          <div style={{
            ...
            padding: '12px 16px',
            borderBottom: '1px solid rgba(45,82,105,0.35)',
            background: '#0d1c24',
          }}>
            <svg width="16" height="16" viewBox="0 0 12 12" fill="none" style={{ flexShrink: 0 }}>
              <circle cx="5" cy="5" r="3.5" stroke="rgba(255,255,255,0.35)" strokeWidth="1.3" />
              <path d="M8 8l2.2 2.2" stroke="rgba(255,255,255,0.35)" strokeWidth="1.3" strokeLinecap="round" />
            </svg>
            <input
              ref={mobileInputRef}
              value={query}
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {results.map(r => (
              ...
              >
                <span style={{ fontFamily: 'var(--font-body)', fontSize: 15, color: 'rgba(255,255,255,0.88)', flex: 1, minWidth: 0 }}>
                  {r.nombre}
                </span>
                <span style={{
            ...
            {query.trim() !== '' && results.length === 0 && (
              <p style={{ padding: '20px', fontFamily: 'var(--font-mono)', fontSize: 13, color: 'rgba(255,255,255,0.25)', textAlign: 'center' }}>
                Sin resultados
              </p>
            )}
          </div>
...
function SearchDropdown({ results, onSelect }: { results: { id: string; nombre: string; grupo: string }[]; onSelect: (id: string) => void }) {
  return (
    <div style={{
